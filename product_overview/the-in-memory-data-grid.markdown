...
Since the space is memory-based, it is essential to verify that it does not overflow and crash. The space has a number of [facilities]({%latestjavaurl%}/memory-management-facilities.html) to manage its memory and make sure it does not overflow.
