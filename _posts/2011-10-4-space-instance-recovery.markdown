...
{refer}For more info refer to [Synchronous Replication Behavior During Recovery](/xap96/2012/04/14/synchronous-replication.html#Behavior During Recovery) and [Asynchronous Replication Behavior During Recovery](/xap96/2012/04/13/asynchronous-replication.html#Behavior During Recovery){refer}
...
There is special handling for [Transient Entries](/xap96/2012/12/11/transient-entries.html) (`persist=false`), since they can't be persisted - they are always recovered from the primary.
...
Any other failure - [SpaceMemoryShortageException](/xap96/2012/12/19/all-in-cache-cache-policy.html#SpaceMemoryShortageException), Database not available etc. is retried 3 times before failing.
...
For further info and configuration options see [Distributed Databases](/xap96/2012/12/28/direct-persistency.html#Distributed Databases)
