...
GigaSpaces XAP provides a Mirror Service as a means to achieve [reliable asynchronous persistency](/xap96/2013/07/28/asynchronous-persistency-with-the-mirror.html).
...
GigaSpaces XAP provides two extension points (`SpaceDataSource`/`SpaceSynchronizationEndpoint` to data source integration. GigaSpaces provides a built-in [Hibernate](/xap96/2013/03/03/hibernate-space-persistency.html) implementation of these extensions. For implementation details please refer to [Space Persistency APIs Implementation ](/xap96/2012/12/30/space-persistency.html) in the Programmer's Guide.
...
{% whr %}
{refer}**Next chapter:** [Product Architecture](/xap96/2011/10/02/product-architecture.html){refer}
