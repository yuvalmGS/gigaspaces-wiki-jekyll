...
Projection supported using a [SQLQuery](/xap96/2013/07/21/sqlquery.html) or [Id Queries](/xap96/2013/07/21/id-queries.html). Below a simple example reading a `Person` object where only the 'firstName' and 'lastName' properties are returned with the query result array. All other `Person` properties will not be returned:
...
The [SpaceDocument](/xap96/2013/05/25/document-api.html) support projections as well:
...
You can use projections with the [Notify Container](/xap96/2013/04/24/notify-container.html) when subscribing to notifications, or with the [Polling Container](/xap96/2013/04/12/polling-container.html) when consuming space objects. You can also create a [Local View](/xap96/2013/04/03/local-view.html) with templates or a `View` using projections. The local view will maintain the relevant objects, but with the projected data - only with the projected properties.
...
1. You can't use a projection on [Local Cache](/xap96/2013/04/08/local-cache.html) as the local cache needs to contain the fully constructed objects, and reconstructing it locally with projection will only impact performance.
