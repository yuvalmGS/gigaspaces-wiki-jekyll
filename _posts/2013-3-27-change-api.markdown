...
The change operation may receive any [query template](/xap96/2013/07/21/querying-the-space.html) for matching a single or multiple objects that needs to be changed by the operation.
...
The `change` operation has the same semantics as regular space `update` operation when it comes to [Optimistic Locking](/xap96/2011/11/15/optimistic-locking.html). It will increase the version of the changed object and the expected version can be specified in the id query when optimistic locking is needed.
...
{refer}For more information on how to implement it please refer to [Change API Advanced](/xap96/2013/03/26/change-api-advanced.html){refer}

# Change and Replication Filter

Change passes through [Replication Filter](/xap96/2011/08/09/cluster-replication-filters.html) like other operations and for example can be discarded on replication level.
{refer}For more information on how to handle change in replication filters please refer to [Change API Advanced](/xap96/2013/03/26/change-api-advanced.html){refer}
