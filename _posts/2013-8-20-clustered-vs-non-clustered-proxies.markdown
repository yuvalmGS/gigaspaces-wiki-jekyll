...
When deploying a Processing Unit(PU) configured with an embedded [Space](/xap96/2012/12/19/the-space-component.html) with a clustered SLA or when running a remote clustered space, a clustered `GigaSpace` proxy is created. 
...
You can use the `clustered` property to control this behavior or use the API to use a non-clustered embedded proxy to create a clustered proxy. This allows the collocated business logic to access the entire cluster to perform cluster wide operations. Clustered and Non-Clustered proxies may be used with Collocated a [Task](/xap96/2013/06/01/task-execution-over-the-space.html), [Service](/xap96/2013/08/25/executor-based-remoting.html), [Notify Container](/xap96/2013/04/24/notify-container.html) , [Polling Container](/xap96/2013/04/12/polling-container.html) and any other Collocated business logic.
...
When using a Spring based `pu.xml` to construct the [GigaSpace](/xap96/2013/08/26/the-gigaspace-interface.html) bean to be injected into the relevant other beans the following should be used to create a clustered and a non-clustered `GigaSpace` bean:
...
Our Service using the [@PostPrimary](/xap96/2012/12/19/the-space-component.html#Primary Backup Notifications) to decorate the method that constructs the clustered proxy from the non-clustered proxy and also the [@ClusterInfoContext](/xap96/2012/12/17/obtaining-cluster-information.html) that provides information about the cluster topology and the local partition ID. 
...
Our `DistributedTask` implements the [ClusterInfoAware](/xap96/2012/12/17/obtaining-cluster-information.html). This allows it to be injected with the `ClusterInfo` that provides information about the cluster topology and the local partition ID. Here is how the `DistributedTask` looks like:
...
Our Event Container (notify container) using the [@PostPrimary](/xap96/2012/12/19/the-space-component.html#Primary Backup Notifications) to decorate the method that constructs the clustered proxy from the non-clustered proxy and also the [@ClusterInfoContext](/xap96/2012/12/17/obtaining-cluster-information.html) that provides information about the cluster topology and the local partition ID. 
