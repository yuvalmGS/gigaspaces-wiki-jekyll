...
A secured embedded Space protects access (to data) which is granted only to users with sufficient privileges. When a remote Space proxy connects to a secured Space, it must provide security credentials (usually username and password - see [Custom Security](/xap96/2013/06/02/custom-security.html) for extensions).
...
{refer}[Configuring Processing Unit Elements](/xap96/2012/09/04/configuring-processing-unit-elements.html), [Configuring the Space](/xap96/2012/12/19/the-space-component.html){refer}
...
Of course, having the username and password exposed (in pu.xml/pu.properties) isn't that "secure". A preferred usage would be to supply the credentials **during deployment**. The _UI_, _CLI_ and _Admin API_ provide a comprehensive support for deploying a secured processing unit (see [Security Administration](/xap96/2009/09/17/security-administration.html)).
...
{refer}[Local Cache](/xap96/2011/11/11/local-view-and-local-cache.html){refer}
...
{refer}[Local View](/xap96/2011/11/11/local-view-and-local-cache.html){refer}
...
{refer}[OpenSpaces Space Filter support](The Space Component#SpaceFilters), [Space Filters](Space Filters){refer}
...
{refer}[Executors](/xap96/2013/06/01/task-execution-over-the-space.html){refer}
...
{refer}[Executors Based Remoting](/xap96/2013/08/25/executor-based-remoting.html){refer}
...
{refer}[Event Driven Remoting](/xap96/2013/05/25/event-driven-remoting.html){refer}
...
{refer}[JDBC Driver](/xap96/2011/06/03/jdbc-driver.html){refer}
...
{% infosign %} An alternative way of querying the Space using SQL syntax is the [SQLQuery](/xap96/2013/07/21/sqlquery.html) class, with a privileged GigaSpace proxy.
