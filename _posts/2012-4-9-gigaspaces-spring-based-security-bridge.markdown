...
{% info %}
 For convenience, the template properties `config/security/spring-security.properties` can be renamed to `security.properties` or referenced using a system property (see [Security Configurations](/xap96/2010/05/05/security-configurations.html)). 
{% endinfo %}
...
GigaSpaces provides a set of [Security Authorities](/xap96/2011/01/27/security-authorities.html) which can be stored as a String in any external directory. Much like the Spring's Security `GrantedAuthority` s which translate to and from a String.
