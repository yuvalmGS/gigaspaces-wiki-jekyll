...
The above example registers with the space for write notifications using the provided template (a `Data` object with its processed flag set to `false`). If a notification occurs, the `SimpleListener` is invoked. Registration for notifications is performed on the configured [GigaSpace](/xap96/2013/08/26/the-gigaspace-interface.html) bean. (In this case, if working in a clustered topology, the notification is performed directly on the cluster member.)
...
{% exclamation %} When using transactions, only the event listener operations are rolled back. The notifications are not sent again in case of a transaction rollback. In case this behavior is required, please consider using the [Polling Event Container](/xap96/2013/04/12/polling-container.html).
...
{% infosign %} For full FIFO support, the actual template also has to be marked as FIFO. For more details, refer to the [Space FIFO support](/xap96/2013/02/13/fifo-support.html) section.
...
Lease Renewal Manager provides a systematic lease renewal and management framework and is used by Notify Container for managing the notification registration leases. Leases are managed in an automatic manner without any application intervention. More information regarding Lease Renewal Manager can be found [here](/xap96/2013/02/27/leases---automatic-expiration.html#LeaseRenewalManager).
...
{% tip %}
To make sure the client application will manage to reconnect automatically to the space cluster once it was restarted you may want to change the default configuration. See the [Proxy Connectivity](/xap96/2013/03/18/proxy-connectivity.html) for details.
{% endtip %}
...
For further details see [Durable Notifications](/xap96/2012/04/16/durable-notifications.html).
