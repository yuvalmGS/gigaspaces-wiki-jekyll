...
{% compositionsetup %}
{% info %}
 **Summary:** {excerpt}Example application that demonstrates the concept of Processing Unit.{excerpt}{whr:#003366}
...
{% togglecloak id=1 %}  ~**Click Here to view the example code (the full source code can be found at `<GigaSpaces Root>\cpp\examples\CppService\CppService.cpp`)**~{% endtogglecloak %}
{% gcloak 1 %}
...
The CPP PU following the standard [Processing Unit Configuration](/xap96/2013/03/11/the-processing-unit-structure-and-configuration.html).

{% togglecloak id=2 %}  ~**Click Here to view the CPP PU Configuration File**~{% endtogglecloak %}
{% gcloak 2 %}
