...
- **GigaSpaces Container (GSC)** \- a generic container that can run one or more [processing units](/xap96/2013/02/14/packaging-and-deployment.html). A space instances usually runs within processing unit. The GSC is launched on each machine that participates in the space cluster, and hosts the space instances.
...
For more details on how to configure the replication mechanisms of the Space, please refer to [this page](/xap96/2011/11/20/replication.html) in the [Administrator's Guide](/xap96/2008/09/07/administrator's-guide.html). 
{% endinfo %}
