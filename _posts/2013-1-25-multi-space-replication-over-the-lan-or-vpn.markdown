...
{refer}For full details of how to enable the bootstrap mechanism refer to [Replication Gateway Bootstrapping Process](/xap96/2012/09/24/replication-gateway-lan-bootstrapping-process.html){refer}
...
{refer}For full details of how to add and remove gateway targets during runtime refer to [Changing Multi-Site Deployment during Runtime](/xap96/2012/08/01/changing-multi-site-deployment-during-runtime.html){refer}
{% exclamation %} This link refers to multi-site replication, where replication is done indirectly via local gateway delegator to the target gateway sink. However the subject in this context is the same for both cases.
