...
{% tip %}
**Summary:** {excerpt}In the pessimistic locking approach, your program must explicitly obtain a lock using a transaction on one or more objects before making any changes.{excerpt}
**Contents:**
...
To enforce a sole reader and sole updater for the object we explicitly lock the object via a transaction using the [ReadModifiers.EXCLUSIVE_READ_LOCK](http://www.gigaspaces.com/docs/JavaDoc9.6/index.html?com/j_spaces/core/client/ReadModifiers.html). 
...
{% exclamation %} Having the above executed at the space side via [Executor Based Remoting](/xap96/2013/08/25/executor-based-remoting.html) or [Task Executors](/xap96/2013/06/01/task-execution-over-the-space.html) is highly recommended. This will shorten the transaction time period since all space operations will be executed against a colocated space.
