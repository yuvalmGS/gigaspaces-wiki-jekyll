...
{% exclamation %} For a quick start follow the [Elastic Processing Unit deployment example](/xap96/2013/02/21/deploying-onto-the-service-grid.html#ElasticProcessingUnitDeploymentusingtheAdminAPI).
...
- Multiple Eager EPUs can run on the same [Lookup Service](/xap96/2011/09/07/the-lookup-service.html) but on different machines. Machines are marked by starting a Grid Service Agent with a specific zone (With a commandline argument -Dcom.gs.zones=zone1).
...
Each machine requires a single running [GigaSpaces Agent](/xap96/2012/06/05/the-grid-service-agent.html). The example below shows how to start a new GigaSpaces agent. The command line parameters instruct the agents to communicate with each other and start the specified amount of managers. It does not start any containers automatically. The EPU starts containers on demand.
...
Configure the EPU scale config to use `dedicatedManagementMachines`, and reduce the `reservedMemoryCapacityPerMachine`. For more information consult the {javadoc-no-frames:org/openspaces/admin/pu/elastic/config/DiscoveredMachineProvisioningConfigurer.html|discovered machine provisioning configuration JavaDoc}.
...
The EPU supports automatic Virtual Machines provisioning through custom plugins. The plugins are open source [http://svn.openspaces.org/cvi/trunk](http://svn.openspaces.org/cvi/trunk) and provide an implementation of {javadoc-no-frames:org/openspaces/admin/pu/elastic/ElasticMachineProvisioningConfig.html|ElasticMachineProvisioningConfig} and {javadoc-no-frames:org/openspaces/grid/gsm/machines/plugins/ElasticMachineProvisioning.html|ElasticMachineProvisioning}.

When deploying an EPU pass an instance of {javadoc-no-frames:org/openspaces/admin/pu/elastic/ElasticMachineProvisioningConfig.html|ElasticMachineProvisioningConfig} as the {javadoc-no-frames:org/openspaces/admin/pu/elastic/topology/ElasticDeploymentTopology.html|machineProvisioning} deployment property.
...
When deploying Gigaspaces XAP on the management machine(s) place the plug-in JAR file under `/gigaspaces-xap/lib/platform/esm` folder. The ESM then loads classes specified by the `machineProvisioning` configuration. These classes need to implement either {javadoc-no-frames:org/openspaces/grid/gsm/machines/plugins/ElasticMachineProvisioning.html|ElasticMachineProvisioning} or {javadoc-no-frames:org/openspaces/grid/gsm/machines/plugins/NonBlockingElasticMachineProvisioning.html|NonBlockingElasticMachineProvisioning}. That class must also implement {javadoc-no-frames:org/openspaces/core/bean/Bean.html|Bean} which has resemblance to the Spring Bean.
...
This delay can be reduced by modifying configuration settings as explained in [Failure Detection](/xap96/2012/07/11/failure-detection.html). Overriding the default value of these context properties is achieved with the {javadoc-no-frames:org/openspaces/admin/pu/elastic/topology/ElasticDeploymentTopology.html|addContextProperty} deployment property. For example: 
...
Here are the main configuration properties you may use with the {javadoc-no-frames:org/openspaces/admin/space/ElasticSpaceDeployment.html|ElasticSpaceDeployment} and the {javadoc-no-frames:org/openspaces/admin/pu/elastic/ElasticStatefulProcessingUnitDeployment.html|ElasticStatefulProcessingUnitDeployment}:
...
|scale| {javadoc-no-frames:org/openspaces/admin/pu/elastic/config/EagerScaleConfig.html|EagerScaleConfig} or {javadoc-no-frames:org/openspaces/admin/pu/elastic/config/ManualCapacityScaleConfig.html|ManualCapacityScaleConfig} |Enables the specified scale strategy, and disables all other scale strategies.| |No|
...
Here are the main configuration properties you may use with the {javadoc-no-frames:org/openspaces/admin/pu/elastic/config/EagerScaleConfig.html|EagerScaleConfig} and the {javadoc-no-frames:org/openspaces/admin/pu/elastic/config/ManualCapacityScaleConfig.html|ManualCapacityScaleConfig}:
...
[Download the example](/attachment_files/EPUScaleDemo.zip) and extract it.
...
Log file: C:\gigaspaces-xap-premium-8.0.0-ga\logs\2011-03-01~12.34-gigaspaces-service-127.0.0.1-6760.log
Created Admin - OK!
