...
{% infosign %} For more information, see [Nested Object Queries](/xap96/2013/07/21/sqlquery.html#Nested Properties)
...
{% exclamation %} See the [Free Text Search](/xap96/2013/07/21/sqlquery.html#Free Text Search) section for more details.
...
{% info %}
All segments of a compound index must have consistent `hashCode()` and `equals()` methods implementation and must use the OBJECT [StorageType](/xap96/2013/03/15/storage-types---controlling-serialization.html).
{% endinfo %}
...
A Compound Space Index of a [space Document](/xap96/2013/05/25/document-api.html) can be described by `pu.xml` configuration file. Example:
