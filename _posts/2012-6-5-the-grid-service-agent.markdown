...
The Grid Service Agent (GSA) acts as a process manager that can spawn and manage Service Grid processes (Operating System level processes) such as [The Grid Service Manager](/xap96/2011/09/07/the-grid-service-manager.html), [The Grid Service Container](/xap96/2013/03/18/the-grid-service-container.html), and [The Lookup Service](/xap96/2011/09/07/the-lookup-service.html).

Usually, a single GSA is run per machine (on a lookup group or lookup locator level, see more in  [The Lookup Service](/xap96/2011/09/07/the-lookup-service.html)). The GSA allows to spawn a [Grid Service Manager](/xap96/2011/09/07/the-grid-service-manager.html), [Grid Service Container](/xap96/2013/03/18/the-grid-service-container.html), [Lookup Service](/xap96/2011/09/07/the-lookup-service.html), and custom processes. Once spawned, the GSA assigns a unique id for it and manages its lifecycle. The GSA will restart the process if it exits, or if a specific console output has been encountered (for example, OutOfMemoryError).

The GSA exposes the ability to start, restart, and kill a process either using the [Administration and Monitoring API](/xap96/2013/05/06/administration-and-monitoring-api.html) or the GigaSpaces UI.

{% lampon %} Although [The Grid Service Manager](/xap96/2011/09/07/the-grid-service-manager.html), [The Grid Service Container](/xap96/2013/03/18/the-grid-service-container.html), and [The Lookup Service](/xap96/2011/09/07/the-lookup-service.html) can be started on their own using their respective scripts, it is preferable that they will be started using the GSA thus allowing to easily monitor and manage them.

# Process Management

The GSA manages Operating System processes. There are two types of process management, local and global.

Local processes simply start the process type (for example, a [Grid Service Container](/xap96/2013/03/18/the-grid-service-container.html)) without taking into account any other process types running by different GSAs.

Global processes take into account the number of process types ([Grid Service Manager](/xap96/2011/09/07/the-grid-service-manager.html) for example) that are currently running by other GSAs (within the same lookup groups or lookup locators). It will automatically try and run at least X number of processes **across** all the different GSAs (with a maximum of 1 process type per GSA). If a GSA running a process type that is managed globally fails, another GSA will identify the failure and start it in order to maintain at least X number of global process types.

# Starting the GSA

In order to start the GSA, the `<GSHOME>/bin/gs-agent.(sh/bat)` can be used. The GSA, by default, will start 2 local [Grid Service Container](/xap96/2013/03/18/the-grid-service-container.html)s, and manage 2 global [Grid Service Manager](/xap96/2011/09/07/the-grid-service-manager.html)s and 2 global [Lookup Service](/xap96/2011/09/07/the-lookup-service.html)s.
...
By default, the GSA is started with 2 local [Grid Service Container](/xap96/2013/03/18/the-grid-service-container.html)s, and manage 2 global [Grid Service Manager](/xap96/2011/09/07/the-grid-service-manager.html) and 2 global [Lookup Service](/xap96/2011/09/07/the-lookup-service.html). This is the equivalent of starting the GSA with the following parameters:
...
|:-------------|:----------|:-------------------|:-------------------|
|gsc|Defines a [Grid Service Container](/xap96/2013/03/18/the-grid-service-container.html)|gsc.xml|gsc.properties|
|gsm|Defines a [Grid Service Manager](/xap96/2011/09/07/the-grid-service-manager.html)|gsm.xml|gsm.properties|
|lus|Defines a [Lookup Service](/xap96/2011/09/07/the-lookup-service.html)| lus.xml|lus.properties|
|gsm\_lus|Defines a [Grid Service Manager](/xap96/2011/09/07/the-grid-service-manager.html) and [Lookup Service](/xap96/2011/09/07/the-lookup-service.html) within the same JVM|gsm\_lus.xml|gsm_lus.properties|
|esm|Defines an Elastic Service Manager which is required for deploying [Elastic Processing Unit](/xap96/2013/05/25/elastic-processing-unit.html)|esm.xml|esm.properties |
...
When starting a [Lookup Service](/xap96/2011/09/07/the-lookup-service.html) and other services in unicast mode (not multicast), it means that specific machines will be the ones that will run the [Lookup Service](/xap96/2011/09/07/the-lookup-service.html). This means that on the machines running the LUS, the following command will be used (assuming other defaults are used for GSM and GSC):
