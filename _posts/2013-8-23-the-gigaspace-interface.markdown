...
|:------|:----------|:-------|:------------|
|space| [The Space bean](/xap96/2012/12/19/the-space-component.html). This can be an embedded space , remote space , local view or local cache proxy. |YES| |
|clustered|Boolean. [Cluster flag](/xap96/2013/08/20/clustered-vs-non-clustered-proxies.html). Controlling if this GigaSpace will work with a clustered view of the space or directly with a cluster member. By default if this flag is not set it will be set automatically by this factory. It will be set to true if the space is an embedded one AND the space is not a local cache proxy. It will be set to false otherwise (i.e. the space is not an embedded space OR the space is a local cache proxy)| NO | true for remote proxy , false for embedded proxy|
...
|default-write-lease| Numerical Value. Sets the default [space object lease](/xap96/2013/02/27/leases---automatic-expiration.html) (TTL) for `write(Object)` operation. |NO| FOREVER. TimeUnit:millsec|
...
A simple way to use the embedded space in a clustered architecture would be by deploying a [clustered space](/xap96/2013/03/30/deploying-and-interacting-with-the-space.html) or packaging your application as a [Processing Unit](/xap96/2013/02/14/packaging-and-deployment.html) and deploy it using the relevant SLA. 

## GigaSpace with a Local (Near) Cache

The `GigaSpace` support [Local Cache](/xap96/2013/04/08/local-cache.html) (near cache) configuration. This provides a front-end client side cache that will be used with the `read` operations implictly. The local cache will be loaded on demand or when you perform a `read` operation and will be updated implictly by the space. 
...
The `GigaSpace` support [Local View](/xap96/2013/04/03/local-view.html) configuration. This provides a front-end client side cache that will be used with any `read` or `readMultiple` operations implictly. The local view will be loaded on start and will be updated implictly by the space. 
...
- In case the space has been fully terminated (no backup or primary instances running any more) the client space proxy will try to reconnect to the space up to a predefined timeout based on the [Proxy Connectivity](/xap96/2013/03/18/proxy-connectivity.html) settings. If it fails to reconnect, an error will be displayed.
...
The {javadocos:org/openspaces/core/GigaSpace|GigaSpace} interface includes the following main operations:
...
{: .table .table-bordered}
|[Data Query operations](/xap96/2013/07/21/sqlquery.html)|Data Insert and Update operations|[Business logic execution operations](/xap96/2013/06/01/task-execution-over-the-space.html)|Data removal operations|
|:--|:--|:--|:--|
|read{% wbr %}readMultiple{% wbr %}[iterator](/xap96/2013/08/20/paging-support-with-space-iterator.html)|write{% wbr %}writeMultiple{% wbr %}   [change](/xap96/2013/03/27/change-api.html) |execute{% wbr %}executorBuilder|clean{% wbr %}clear{% wbr %}take{% wbr %}takeMultiple|

{include:POJO Code Snippets}
...
When configuring a [Space](/xap96/2012/12/19/the-space-component.html) with an embedded clustered space or with a remote clustered space, a clustered `GigaSpace` proxy is created. A clustered proxy is a smart proxy that performs operations against the entire cluster when needed. 
...
The [Clustered vs Non-Clustered Proxies](/xap96/2013/08/20/clustered-vs-non-clustered-proxies.html) provide more details how to use Clustered Proxies.
...
{% tip %}
See more examples for the `GigaSpace` interface usage with the [POJO Support](/xap96/2013/03/12/pojo-support.html) section.
{% endtip %}
...
Writing an object into a space might generate [notifications](/xap96/2013/04/24/notify-container.html) to registered objects.

## Time To Live - Lease

To write an object into the space with a limited time to live you should specify [a lease value](/xap96/2013/02/27/leases---automatic-expiration.html) (in millisecond). The object will expire automatically from the space. 

{% highlight java %}
gigaSpace.write(myObject, 10000)
{% endhighlight %}

You can [register for notifications](/xap96/2013/04/24/notify-container.html) having a listener triggered when the object has been expired.
...
Alternatively, you can use the [change](/xap96/2013/03/27/change-api.html) operation and update specific fields or even nested fields or modify collections and maps without having to supply the entire collection or map upon such update. The following `change` operation example is equivalent to the previous partial update operation.
...
{% tip %}
The `readByID` and `readByIDs` have a special performance optimization when running a [Local Cache](/xap96/2013/04/08/local-cache.html) or [Local View](/xap96/2013/04/03/local-view.html).
{% endtip %}

 See the [Id Queries](/xap96/2013/07/21/id-queries.html) for details.

## Template Based

The template is a POJO of the desired entry type, and the properties which are set on the template (i.e. not null) are matched against the respective properties of entries of the same type in the space. Properties with null values are ignored (not matched). See the [Template Matching](/xap96/2013/03/20/template-matching.html) for details.

## SQL Based

The [SQLQuery](/xap96/2013/07/21/sqlquery.html) class is used to query the space using SQL-like syntax. The query statement includes only the `WHERE` statement part - the selection aspect of a SQL statement is embedded in other parameters for a SQL query. See the [SQLQuery](/xap96/2013/07/21/sqlquery.html) for details.

## Space Iterator

The [IteratorBuilder](http://www.gigaspaces.com/docs/JavaDoc9.6/index.html?org/openspaces/core/IteratorBuilder.html) with the [GSIterator](http://www.gigaspaces.com/docs/JavaDoc9.6/index.html?com/j_spaces/core/client/GSIterator.html) allows you to iterate over large amount of space objects in a **paging approach**. It avoids the need to retrieve the entire result set in one batch as the `readMultiple` since it is fetching the result set in batches. This optimizes the resource utilization (memory and CPU) involved when executing the query both at the client and server side. See the [Paging Support with Space Iterator](/xap96/2013/08/20/paging-support-with-space-iterator.html) for details.

# readIfExists and read Operations

The two forms of the `read` operations query the space for an object that matches the template/[SQLQuery](/xap96/2013/07/21/sqlquery.html) provided. If a match is found, a copy of the matching object is returned. If no match is found, `null` is returned. Passing a `null` reference as the template will match any object. 

Any matching object can be returned. Successive read requests with the same template may or may not return equivalent objects, even if no intervening modifications have been made to the space. Each invocation of `read` may return a new object even if the same object is matched in the space. If you would like to read objects in the same order they have been written into the space you should perform the read objects in a [FIFO mode](/xap96/2013/02/13/fifo-support.html).
...
If you would like to take objects from the space in the same order they have been written into the space you should perform the take objects in a [FIFO mode](/xap96/2013/02/13/fifo-support.html).

Taking an object from the space might generate [notifications](/xap96/2013/04/24/notify-container.html) to registered objects/queries.
...
Asynchronous `write` operation can be implemented using a [Task](/xap96/2013/06/01/task-execution-over-the-space.html), where the `Task` implementation include a write operation. With this approach the `Task` is sent to the space and executed in an asynchronous manner. The write operation itself will be completed once both the primary and the backup will acknowledge the operation. This activity will be performed as a background activity from the client perspective. 
