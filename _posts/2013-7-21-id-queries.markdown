...
The space can be queried for entries using [Template Matching](/xap96/2013/03/20/template-matching.html) or [SQLQuery](/xap96/2013/07/21/sqlquery.html), but sometimes we know the exact id of the entry we need and prefer a faster solution. This is where id-based queries come handy.
...
You can specify which properties should be populated when the result is created the [Projection API](/xap96/2013/08/13/getting-partial-results-using-projection-api.html).

{% tip %}
See [Parent Child Relationship](/xap96/2011/02/06/parent-child-relationship.html) for a full usage example of the `readByIds` operation.
