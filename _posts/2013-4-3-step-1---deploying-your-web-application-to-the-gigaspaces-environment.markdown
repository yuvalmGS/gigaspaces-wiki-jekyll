...
{excerpt}
**Step summary**: Learn how to **deploy an existing stadard web application into the GigaSpaces environment**, benefitting from the platform's self healing and dynamic load blanacing capabilities{excerpt}
...
In order to deploy the application onto the GigaSpaces infrastructure (Service Grid), you will need to start the  [GigaSpaces agent](/xap96/2012/06/05/the-grid-service-agent.html) on your local machine. The agent automatically manages the GigaSpaces processes for you, and will start one GSM and two GSCs by default. This is done by calling `<gs root>/bin/gs-agent.(sh/bat)`. 
...
One of the key advantages of using GigaSpaces to deploy and manage your web application is the GigaSpaces [Service Grid infrastructure](/xap96/2011/10/02/product-architecture.html#ProductArchitecture-SLADrivenContainer). The GigaSpaces Service Grid will monitor the application running instances (in GigaSpaces these are termed [**Processing Units**](/xap96/2012/04/09/the-application-layer.html#TheApplicationLayer-ProcessingUnit)) and the containers on which they run. It has two major roles, which are together termed the application's **SLA**: 
...
{% color green %}Step Two{% endcolor %}
 - [Enabling HTTP Session Failover & Fault Tolerance](/xap96/2013/04/03/step-2---enabling-http-session-failover-and-fault-tolerance.html) - Shows how to back your HTTP session with the Space to achieve highly available and fault-tolerant HTTP session for your web application.
