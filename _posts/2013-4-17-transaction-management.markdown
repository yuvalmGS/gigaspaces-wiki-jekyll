...
{% exclamation %} In order to make [The GigaSpace Interface ](/xap96/2013/08/26/the-gigaspace-interface.html) transactional, the transaction manager must be provided to it when constructing the GigaSpace bean.
...
More information regarding Lease Renewal Manager can be found [here](/xap96/2013/02/27/leases---automatic-expiration.html#LeaseRenewalManager).
...
{% tip %}
See the [JTA-XA Example](SBP:JTA-XA Example) for fully running demonstration how to integrate GigaSpaces with an external JMS Server.
{% endtip %}
