...
Additional info about GigaSpaces proxy can be found [here](/xap96/2013/03/18/proxy-connectivity.html)
...
OpenSpaces wraps the GigaSpaces core product with Spring which enables Spring configuration and Spring life cycle to GigaSpaces applications. Some additional info about OpenSpaces is [here](/xap96/2012/04/09/openspaces-faq.html).
...
GigaSpaces components communicate using internal communications adaptor, LRMI (Light Remote Method Invocation), which is based on NIO communication protocol. More information about lrmi is [here](/xap96/2013/05/08/communication-protocol.html).
{% endtabcontent %}
...
GigaSpaces applications are running as part of a GigaSpaces runtime container and packaged using the structure described [here](/xap96/2013/03/11/the-processing-unit-structure-and-configuration.html).
Application jars/classes are packaged in different folders and some of the classes could be loaded as part of GigaSpaces container (GSC's). There are multiple class loaders involved when an application is running. More information about the class loaders and their hierarchy is [here](/xap96/2013/03/11/the-processing-unit-structure-and-configuration.html#Class Loaders).
{% endtabcontent %}
{% tabcontent Space %}
...
Space filters are described [here](/xap96/2013/03/06/space-filters.html)
...
GigaSpaces persistence options are explained [here](/xap96/2013/05/25/persistency.html). One of the packaged External Data Source implementations uses Hibernate and it is called Hibernate External Data Source which is described [here](Hibernate External Data Source).
...
GigaSpaces supports SQL queries on the data in space and logger corresponds to this [functionality](/xap96/2013/07/21/sqlquery.html).
...
More information about LRU policy and Eviction behavior is [here](/xap96/2012/12/19/lru-cache-policy.html)
...
Notifications are a mechanism that can be used to identify events related to space data (write, update, take, etc). Notifications are typically used with a [Notify Container](/xap96/2013/04/24/notify-container.html).
Another way notifications can be used is thru Session based messaging which is discussed [here](/xap96/2013/04/24/session-based-messaging-api.html).
...
FIFO functionality is applicable for writes, reads and events (notifications) and discussed [here](/xap96/2013/02/13/fifo-support.html).
...
When a cluster topology is replicated, replication functionality is enabled. More information about topologies is [here](/xap96/2012/12/24/space-topologies.html).
Replication between spaces is one of the core features of GigaSpaces and is explained in detail [here](/xap96/2011/11/20/replication.html).
...
When cluster uses partitioned topology, data is partitioned across multiple instances of spaces. More information about topologies is [here](/xap96/2012/12/24/space-topologies.html).
...
Logger corresponding to GigaSpaces POJO support, more info [here](/xap96/2013/03/12/pojo-support.html).
...
Logger corresponding to XA Transaction manager running in the space, more information [here](/xap96/2013/04/17/transaction-management.html).
...
Logger for Jini Distributed Transaction manager, more information [here](/xap96/2013/04/17/transaction-management.html#DistributedJiniTransactionManager).
...
SpaceURL and its constraints are explained [here](/xap96/2012/05/20/space-url.html).
...
Space notifications support multicast mode and this logger corresponds to this [functionality](/xap96/2013/04/24/session-based-messaging-api.html#AdvancedOptions)
{% endtabcontent %}
{% tabcontent Runtime %}
...
Logger corresponding to security of GigaSpaces components. This includes configuration and runtime execution of security functionality. More information regarding GigaSpaces security is [here](/xap96/2012/06/14/security.html).
{% endtabcontent %}
...
Web application support in GigaSpaces is provided using a Jetty container. These loggers correspond to Web Container. More information about GigaSpaces Web Application support is [here](/xap96/2013/03/17/web-jetty-processing-unit-container.html).
{% endtabcontent %}
...
These loggers correspond to Mule integration. More information about Mule integration is here, [Mule ESB](/xap96/2012/08/30/mule-esb.html) and [Mule Processing Unit](/xap96/2012/08/30/mule-processing-unit.html)
{% endtabcontent %}
...
Loggers corresponding to GigaSpaces Management Console/UI. Additional information regarding UI can be found [here](/xap96/2011/04/12/gigaspaces-management-center.html).
{% endtabcontent %}
{% endinittab %}
...
- `com.gigaspaces.logger.**RollingFileHandler**`: A handler that writes formatted output to a file that rolls over if a certain policy is triggered. see [Managing Log Files](/xap96/2010/08/24/managing-log-files.html)
...
`com.gigaspaces.logger.`**`GSSimpleFormatter`**. This formatter class is based on the `java.util.logging.`**`SimpleFormatter`** class. see [Formatting Log Messages](/xap96/2009/07/16/formatting-log-messages.html) for more details. 
