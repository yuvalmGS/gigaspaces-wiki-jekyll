...
The above share the **same** [Space Persistency](/xap96/2012/12/30/space-persistency.html) settings but have different space settings. See the [Hibernate Space Persistency](/xap96/2013/03/03/hibernate-space-persistency.html) for details how to use the built-in `HibernateSpaceSynchronizationEndpoint`.
...
| cluster-config.mirror-service.on-redo-log-capacity-exceeded | Available options:{% wbr %}block-operations - all cluster operations that need to be replicated (write/update/take) are blocked until the redo log size decreases below the capacity. (Users get RedoLogCapacityExceededException exceptions while trying to execute these operations.){% wbr %}drop-oldest - the oldest packet in the redo log is dropped.{% wbr %}See the [Controlling the Replication Redo Log](/xap96/2012/12/19/controlling-the-replication-redo-log.html) for details. | block-operations | 
| cluster-config.mirror-service.redo-log-capacity | Specifies the total capacity of replication packets the redo log can hold for a mirror service replication target.{% wbr %}See the [Controlling the Replication Redo Log](/xap96/2012/12/19/controlling-the-replication-redo-log.html) for details. | 1000000 | 
...
{refer}See the [Space Persistency Properties](/xap96/2012/12/11/space-persistency-advanced-topics.html#Properties) and the [Hibernate Space Persistency](/xap96/2013/03/03/hibernate-space-persistency.html) for full details about the EDS properties the you may configure.{refer}
...
If you wish to change the mirror service name please refer to [Async Persistency - Mirror - Advanced](/xap96/2013/02/14/async-persistency---mirror---advanced.html).
...
- The above configuration constructs a Mirror Service using GigaSpaces built-in [Hibernate Space Persistency](/xap96/2013/03/03/hibernate-space-persistency.html). The `hibernateSpaceSynchronizationEndpoint` should have its `sessionFactory` injected.
...
See the [The Processing Unit Structure and Configuration](/xap96/2013/03/11/the-processing-unit-structure-and-configuration.html) for more information on the processing unit structure.
...
- Use stateless session with the [Hibernate Space Persistency](/xap96/2013/03/03/hibernate-space-persistency.html) configuration. See the `StatelessHibernateSpaceSynchronizationEndpoint`.
...
- Optimize Hibernate mapping and configuration. Using a proper [Hibernate ID generator](/xap96/2012/12/11/space-persistency-advanced-topics.html#HibernateIDGeneration) is crucial for getting optimum performance.
...
The activity of the mirror service can be monitored using the [Administration and monitoring API](/xap96/2013/05/06/administration-and-monitoring-api.html#AdministrationandMonitoringAPI-MonitoringtheMirrorService). This API exposes statistics on operations that were executed by the mirror and can be used to monitor the mirror throughput and health status. 

You may view Mirror and its replication statistics via the [GigaSpaces Management Center](/xap96/2011/04/12/gigaspaces-management-center.html). Move into the Space Browser tab, click the top tree Spaces icon, right click the table columns title area on the right panel, select the columns you would to view as part of the table and click OK.
...
{% tip %}
See also the [Mirror Monitor JMX utility](SBP:Mirror Monitor) for graphical mirror service monitoring via JMX
{% endtip %}

# Advanced Information & Operations

{refer}For more advanced operations and information see [Async Persistency - Mirror - Advanced](/xap96/2013/02/14/async-persistency---mirror---advanced.html).{refer}
