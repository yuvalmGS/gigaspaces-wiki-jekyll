...
![referto.gif](/attachment_files/referto.gif) For the full documentation of the class's methods and constructors, see [Javadoc](http://www.gigaspaces.com/docs/JavaDoc9.6/index.html?com/j_spaces/core/client/SQLQuery.html).
{% endcolumn %}
{% column %}
...
![referto.gif](/attachment_files/referto.gif) For an example of `SQLQuery` with `EventSession`, refer to the [Session Based Messaging API](/xap96/2013/04/24/session-based-messaging-api.html#SQLQuery Template Registration) section.
...
- [Regular Index](/xap96/2013/05/25/indexing.html) and a [Compound Index](/xap96/2013/05/25/indexing.html#Compound Indexing) - Index a single property or multiple properties to improve query execution time.
...
For more information see [Query Nested Properties](/xap96/2013/07/21/query-nested-properties.html).
...
{% plus %} Date properties are often used for comparison (greater/less than). Consider using [extended indexing](/xap96/2013/05/25/indexing.html) to boost performance.
...
You can query the space using the SQL `like` operator or {sunjavadoc:java/util/regex/Pattern|Java Regular Expression} Query syntax. 
...
{% tip %}
The same approach can be implemented also with the [SpaceDocument](/xap96/2013/05/25/document-api.html).
{% endtip %}
...
When having an **AND** query or a template that use two or more fields for matching a [Compound Index](/xap96/2013/05/25/indexing.html#Compound Indexing) may boost the query execution time. The Compound Index should be defined on multiple properties for a specific space class and will be used implictly when a SQL Query or a [Template](/xap96/2013/03/20/template-matching.html) will be using these properties.
...
{refer}For details on how to use the projection API please refer to [Getting Partial Results Using Projection API](/xap96/2013/08/13/getting-partial-results-using-projection-api.html){refer}
...
- Aggregate functions: COUNT, MAX, MIN, SUM, AVG are only supported in sub queries (These are fully supported with the [JDBC API](/xap96/2011/06/03/jdbc-driver.html)). 
- Multiple tables select - This is supported with the [JDBC API](/xap96/2011/06/03/jdbc-driver.html). 
- `DISTINCT` - This is supported with the [JDBC API](/xap96/2011/06/03/jdbc-driver.html). 
...
- Snapshot
- Blocking operations
- [Notifications](/xap96/2013/04/24/session-based-messaging-api.html)
- [GSIterator](/xap96/2013/08/20/paging-support-with-space-iterator.html)
