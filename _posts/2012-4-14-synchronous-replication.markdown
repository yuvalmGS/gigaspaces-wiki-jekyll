...
{% info %}
This page covers details which are specific to synchronous replication which are not covered in the [Replication](/xap96/2011/11/20/replication.html) page, the previous page should be read before this one.
{% endinfo %}
...
{% info %}
The current operating mode of a replication channel can be retrieved via the [Administration and Monitoring API](/xap96/2013/05/06/administration-and-monitoring-api.html) by getting the
{javadoc-no-frames:org/openspaces/admin/space/SpaceInstance.html|Space Instance} {javadoc-no-frames:com/j_spaces/core/filters/ReplicationStatistics.html|Replication Statistics} of the specific replication channel and calling the following getter {javadoc-no-frames:com/j_spaces/core/filters/ReplicationStatistics.OutgoingChannel.html#getOperatingMode()|OutgoingChannel.getOperatingMode()} 
{% endinfo %}
