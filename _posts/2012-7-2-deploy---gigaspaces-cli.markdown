...
A Processing Unit can be easily deployed onto the Service Grid. In order to deploy a Processing Unit, the Processing Unit must follow the [processing unit directory structure](/xap96/2013/03/11/the-processing-unit-structure-and-configuration.html). 
...
{% tip %}
You may use the [GigaSpaces Universal Deployer](SBP:Universal Deployer) to deploy complex multi processing unit applications. 
{% endtip %}
...
{% tip %}
You may use the [Primary-Backup Zone Controller](SBP:Primary-Backup Zone Controller) to deploy primary and backup on specific different zones.
{% endtip %}
