...
The core package of OpenSpaces provides APIs for direct access to a data grid, internally referred to as a "space." The main interface is the GigaSpace, which enables the basic interaction with the data grid. The core components include basic infrastructure support such as [Space](/xap96/2012/12/19/the-space-component.html) construction, simplified API using the [GigaSpace](/xap96/2013/08/26/the-gigaspace-interface.html) interface including [Transaction Management](/xap96/2013/04/17/transaction-management.html) and declarative transaction support. Core components also include support for [Map/Cache](The Map Component) construction and a simplified API using [GigaMap](The GigaMap Interface).
...
The events module includes components for simplified EDA/Service Bus development. These components allow [unified event-handling](/xap96/2012/06/18/data-event-listener.html) and provide two mechanisms for event-generation: a [Polling Container](/xap96/2013/04/12/polling-container.html) uses polling received operations against the space, and a [Notify Container](/xap96/2013/04/24/notify-container.html) which uses the space's built-in notification support.

## Space Based Remoting

The [Remoting](/xap96/2011/05/24/space-based-remoting.html) package provides capabilities for clients to access remote services. Remoting in GigaSpaces XAP is implemented on top of the data grid's clustering model, which provides location transparency, fail-over, and performance to remote service invocations. OpenSpaces implements [remoting](/xap96/2011/05/24/space-based-remoting.html), using the space as the transport layer, similar to [other Spring remoting components](http://static.springframework.org/spring/docs/3.0.x/reference/remoting.html).
...
More information on the usage of the above integrations can be found in the [Programmer's Guide](/xap96/2008/09/16/programmer's-guide.html).
...
Jetty is a very popular web container, which provides support for JEE [web container](/xap96/2012/08/01/web-processing-unit-container.html) specification services such as: Servlet, JavaServer Pages, JavaServer Faces, and others.

The [integration with the Jetty web container](/xap96/2013/03/17/web-jetty-processing-unit-container.html), allows you to run JEE web applications (.war files) in GigaSpaces XAP. 
...
Much like the .NET container, the [CPP Processing Unit](/xap96/2011/06/28/cpp-processing-unit.html) provides a native C++ runtime environment for C++ SBA applications.

## Mule

Mule is a popular open source Enterprise Services Bus implementation in Java. The [Mule container integration](/xap96/2012/08/30/mule-esb.html) allows you to run a Mule application on top of the GigaSpaces XAP, and gain scalability, performance and high-availability, with almost no changes to the Mule application.
{toc-zone}
...
As an application platform, GigaSpaces XAP provides integrated, memory-based runtime capabilities. The core of these capabilities is backed by the space technology - for more information, please refer to [Space - Concepts and Capabilities](/xap96/2011/03/22/concepts.html#Space - Concepts and Capabilities).
...
The main API to access the IMDG service, is the [`GigaSpace` interface](/xap96/2013/08/26/the-gigaspace-interface.html). In addition, one can use the Map API (using the [GigaMap interface](The GigaMap Interface)) to access the IMDG. Please refer to the [Programmer's Guide](/xap96/2008/09/16/programmer's-guide.html) for usage examples.
...
1. [FIFO](/xap96/2013/02/13/fifo-support.html) ordering.
1. [Transaction Management](/xap96/2013/04/17/transaction-management.html).

The core APIs used for messaging are the OpenSpaces [Notify](/xap96/2013/04/24/notify-container.html) and [Polling](/xap96/2013/04/12/polling-container.html) Containers. In addition, there is a [JMS 1.1 implementation](/xap96/2011/03/27/jms-api-support.html) available to be used with existing applications. More information can be found in the [Messaging and Events](Messaging and Events) section.
...
The ability to support a unified clustering model is a direct result of the underlying space-based clustering model. For more information on the concept of space, please refer to [Space - Concepts and Capabilities](/xap96/2011/03/22/concepts.html#Space - Concepts and Capabilities).
...
The [Grid Service Agent (GSA)](/xap96/2012/06/05/the-grid-service-agent.html) acts as a process manager that can spawn and manage Deployment Infrastructure processes (Operating System level processes) such as [Grid Service Manager](/xap96/2011/09/07/the-grid-service-manager.html) and [Grid Service Container](/xap96/2013/03/18/the-grid-service-container.html).

Usually, a single GSA is deployed on each individual machine. The GSA spawns [Grid Service Managers](/xap96/2011/09/07/the-grid-service-manager.html), [Grid Service Containers](/xap96/2013/03/18/the-grid-service-container.html), and other processes. Once a process is spawned, the GSA assigns a unique id for it and manages its life cycle. The GSA will restart the process if it exits abnormally (exit code different than 0), or if a specific console output has been encountered (for example, OutOfMemoryError).

{% lampon %} Though [Grid Service Manager](/xap96/2011/09/07/the-grid-service-manager.html), [Grid Service Container](/xap96/2013/03/18/the-grid-service-container.html), and other processes can be started independently, it is preferable that they be started using the GSA, thus allowing to easily monitor and manage them.
