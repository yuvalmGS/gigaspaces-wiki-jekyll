...
A **Local View** is a [Client Side Cache](/xap96/2012/12/02/client-side-caching.html) that maintains a subset of the master space's data, allowing the client to read distributed data without performing any remote calls or data serialization. 

Data is streamed into the client local view based on predefined criteria (a collection of [SQLQuery](/xap96/2013/07/21/sqlquery.html) objects) specified by the client when the local view is created.
...
{% plus %} For additional client side caching options, refer to [Client Side Caching](/xap96/2012/12/02/client-side-caching.html).
...
Creating a local view is similar to creating a `[GigaSpace](/xap96/2013/08/26/the-gigaspace-interface.html)` instance, except the space should be wrapped with a local view before exposing it as a `GigaSpace`. The local view can be configured via Spring using `LocalViewSpaceFactoryBean` or the `<os-core:local-view>` Spring tag, or in code using `LocalViewSpaceConfigurer`. For exmaple:
...
The Local View is a **Read-Only** client side cache. The following operations are supported - these are served by the local view. The master space is not accessed when calling these:
    - `read` , `readMultiple` , `asyncRead` , `count` , using [Template Matching](/xap96/2013/03/20/template-matching.html), [Id Queries](/xap96/2013/07/21/id-queries.html) or [SQLQuery](/xap96/2013/07/21/sqlquery.html).
    - [Notify Container](/xap96/2013/04/24/notify-container.html) , [Session Based Messaging API](/xap96/2013/04/24/session-based-messaging-api.html). 
    - [IteratorBuilder](/xap96/2013/08/20/paging-support-with-space-iterator.html).
...
Starting with XAP 8.0.6, the local view uses [replication](/xap96/2011/11/20/replication.html) instead of [notifications](/xap96/2013/04/24/session-based-messaging-api.html) to synchronize with the master space. The reason for that change is that replication provides higher reliability than notification. In general this is an implementation detail that should not concern the user, except for the following cases:

- Using a view query on a type/class configured to be [non-replicable](/xap96/2013/06/02/pojo-metadata.html).
- Using a cluster with an [Active-Active](/xap96/2013/05/01/replication-topologies.html) topology, or a non-clustered space.
...
If local view synchronization is done using notifications, the round-trip-time can be configured using the `space-config.dist-cache.events.lease-renew.round-trip-time` custom property. For more information about this setting refer to [Session Based Messaging API](/xap96/2013/04/24/session-based-messaging-api.html).
