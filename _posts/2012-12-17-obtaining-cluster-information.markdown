...
The [Space component](/xap96/2012/12/19/the-space-component.html) implements the `ClusterInfoAware`, and uses it in order to construct an embedded space by mapping `ClusterInfo` properties to Space URL properties (if provided). Here is a mapping from `ClusterInfo` to Space URL properties:
...
When running the processing unit in any of the [runtime modes](/xap96/2011/09/07/deploying-and-running-the-processing-unit.html), `ClusterInfo` can also be used directly within the Spring XML configuration. In a similar manner, properties can be injected. Here is an example of how this can be used:
...
Each of the [runtime modes](/xap96/2011/09/07/deploying-and-running-the-processing-unit.html) supports the injection of `ClusterInfo`. However, when running in [standalone mode](/xap96/2012/09/11/running-in-standalone-mode.html) or [within your IDE](/xap96/2012/01/04/running-and-debugging-within-your-ide.html), the `-cluster` parameter controls the clustering aspects of the processing unit instance(s). Below is a list of the parameters you can specify for the standalone and IDE runtime modes, and how they are mapped the `ClusterInfo` properties: 
...
{refer}For more details on how to use the `-cluster` option, see the [processing unit runtime modes section](/xap96/2011/09/07/deploying-and-running-the-processing-unit.html){refer}
