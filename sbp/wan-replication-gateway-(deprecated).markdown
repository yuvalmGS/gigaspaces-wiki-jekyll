...
- Run gs-agent-SiteA and gs-agent-SiteB scripts on appropriate machines.
...
- Deploy the SiteB space cluster (3,1) using following,
{code}
...
{code}
- Ensure that the spaces are mirror are available in gs-ui. Space Browser tab after everything is deployed will look like below,
...
mvn install
{code}
- Create products (in SiteB) by running `WriteProducts` client using following,
{code}
...
- Write new orders into the system using `WriteOrders` client using following,
{code}
