...
When the Space Class implements the [Externalizable](http://docs.oracle.com/javase/1.5.0/docs/api/java/io/Externalizable.html) interface, `readExternal` and `writeExternal` are called, you may control the stream transferred across the network. The [Externalizable Support (Deprecated)](/xap96/externalizable-support-(deprecated).html) includes details about this advanced option.
