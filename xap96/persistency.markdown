...
The following tables show the correspondence between the Hibernate basic API methods to [GigaSpaces API](/xap96/the-gigaspace-interface.html) and the [GigaMap API](/xap96/map-api.html) methods.
...
The [Moving from Hibernate to Space](http://wiki.gigaspaces.com/wiki/display/SBP/Moving+from+Hibernate+to+Space) best practice includes step by step instructions for moving from Hibernate based application to GigaSpaces Data-Grid as the data access layer. This use Hibernate as the space persistency layer using write-through approach when pushing updates into the database.

{% tip %}
The space can be used as a [Hibernate second level cache](/xap96/gigaspaces-for-hibernate-orm-users.html).
{% endtip %}

# Caching policies and Space Persistency

[Space Persistency](/xap96/space-persistency.html) supports the **All In Cache** and **LRU** [Cache policies](/xap96/memory-management-facilities.html).

## All In Cache Policy

With the [All In Cache policy](ALL IN CACHE-Cache Policy), the assumption is the Space holds the entire data in memory. In this case, the space communicated with the data source at startup, and loads all the data. If data within the space is updated/added/removed, the space is calling the [`SpaceSynchronizationEndpoint`](/xap96/space-synchronization-endpoint-api.html) implementation to update the underlying data source. All data activities leveraging the data in memory.
...
You should have such delegation implemented at the application level or via a "Query Service" that will handle all the queries executed by the different applications. The "Query Service" can be accessed using [Task Executors](/xap96/task-execution-over-the-space.html) or [remoting](/xap96/executor-based-remoting.html). In such a case you should run the space in ALL_IN_CACHE policy and implement one of the [eviction patterns](http://wiki.gigaspaces.com/wiki/display/SBP/Custom+Eviction).
{% endcomment %}

## LRU Cache Policy - Read-Ahead

LRU persistency model is based on the eviction model: **Some** of the data stored In-Memory (based on auto expiration mechanism or explicit data eviction) and **ALL** the data stored on disk where the preferred disk media is a database. You may leverage Hibernate as the mapping layer when data is persist or have a custom persistency mapping implemented leveraging the [Space Data Source API](/xap96/space-data-source-api.html).
...
In both cases (ALL_IN_CACHE and LRU cache policy), you can [customize the data load phase](/xap96/space-persistency-initial-load.html) to speed up the space initialization phase.

# Space Persistency

The space can load data from data sources, store data into data sources, and persist data into a relational data source or any other media via a custom [`SpaceSynchronizationEndpoint`](/xap96/space-synchronization-endpoint-api.html) implementation. [Space Persistency](/xap96/space-persistency.html) a built-in implementation using [Hibernate](/xap96/hibernate-space-persistency.html), to store data in an existing data source and in the space. Data is loaded from the data source during space initialization (via the `SpaceDataSource` implementation), and from then onwards the application works with the space directly. Meanwhile, the data source is constantly updated with all the changes made in the space (via the `SpaceSynchronizationEndpoint` implementation). This is the recommended model.

The [Hibernate Space Persistency](/xap96/hibernate-space-persistency.html) support RDBMS. The [Cassandra Space Persistency](/xap96/cassandra-space-persistency.html) allows applications to leverage NoSQL Cassandra DB having a distributed database infrastructure as an alternative to RDBMS.
