...
| write-mode | `WRITE_THRU` - data is first written to flash and a log entry is created after that. `WRITE_BEHIND` - data & log entry are written to ZetaScale, flush to the disk is done asynchronously.| `WRITE_THRU` | optional |
