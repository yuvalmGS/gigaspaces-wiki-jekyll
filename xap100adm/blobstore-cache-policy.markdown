...
| write-mode | `WRITE_THRU` - data grid writing data immediately into blobstore and synchronously acknowledged after ZetaScale fully commit the operation. `WRITE_BEHIND` - data grid writing data immediately into blobstore. ZetaScale asynchronously commit the operation against the SSD. This option improves write performance but may have consistency issue with a sudden machine failure.| `WRITE_THRU` | optional |
