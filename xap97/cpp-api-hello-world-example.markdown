...
{% refer %}For more details, refer to the [Installing GigaSpaces](/xap96/installing-gigaspaces.html) section.{% endrefer %}

### C++ API installation

{% refer %} See the [Installing CPP API Package](/xap96/installing-cpp-api-package.html) section.{% endrefer %}
...
1. If you want to view the space in the [GigaSpaces Browser](/xap96/gigaspaces-management-center.html), execute `<GigaSpaces Root>\bin\gs-ui.sh/bat`.
...
{% infosign %} The `id` tag defines the `uid` field as the primary key, while the `routing` tag defines the `id` field used for routing. For more details, refer to the [gs.xml reference](/xap96/cpp-api-mapping-file.html) section.
...
{% exclamation %} The space URL in this example is: `jini://localhost/*/mySpace?groups=javaspaces-hellospace`. It is passed to `main` in `run_HelloWorld.sh/.bat)` (see [Space URL](/xap96/space-url.html)).
...
{% refer %}For more details on SQL queries, refer to the [SQLQuery](/xap96/sqlquery.html) section.{% endrefer %}
