...
To solve the performance problems associated with a class serialization, the serialization mechanism allows you to declare that an embedded class is Externalizable. When the ObjectOutputStream writeObject() method is called, it performs the following sequence of actions:
...
{%endhighlight%}
{%endaccord%}
{%accord title=Address.java | parent=acc1%}
{%highlight java%}
