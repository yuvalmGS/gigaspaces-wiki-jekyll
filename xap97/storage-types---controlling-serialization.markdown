...
- Full mode – supports the [JavaSpace specification](http://river.apache.org/doc/spec-index.html). When serialized, non-primitive fields are wrapped with a [MarshalledObject](http://docs.oracle.com/javase/6/docs/api/java/rmi/MarshalledObject.html). The MarshalledObject is de-serialized at the space side before it is stored, allowing you to perform matching using these fields. This mode is slower compared other options.
