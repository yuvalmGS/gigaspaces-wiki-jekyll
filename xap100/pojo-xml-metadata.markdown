...
| `replicate` | This field indicates the replication mode of the object. When a space is defined as replicated, a `true` value for this attribute will replicate objects of this type. {% refer %}For more details, refer to the [Replication]({%currentadmurl%}/replication.html) section. {% endrefer %}{% wbr %}{% exclamation %}To control replication at the object level you should specify a [replication filter]({%currentadmurl%}/cluster-replication-filters.html){% wbr %}|
