...
Building on top of the core API, the Space also provides [higher level services]({{ page.url | remove: 'net' }}/services-on-top-of-the-data-grid.html) onto the application. These services, along with the space's basic capabilities, provide the full stack of middleware features that you can build your application with.
