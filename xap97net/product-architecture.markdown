...
An SLA-Driven Container, also known as the [Service Grid]({%latestjavaurl%}/Service-Grid.html#gsc), is responsible for abstracting the physical characteristics of the host machines from the application deployment.
...
The Service Grid takes a [pre-defined application-required SLA]({%latestjavaurl%}//Service-Grid.html), and makes sure that it is met during deployment and runtime, throughout the application's life-cycle.
...
The [Grid Service Manager (GSM)]({%latestjavaurl%}/Service-grid.html#gsa), is a special infrastructure service, responsible for managing the Service Grid containers. The GSM accepts user deployment and undeployment requests, and provisions the Service Grid cloud accordingly.
...
The GSM service usually contains the [Lookup Service]({%latestjavaurl%}/Service-grid.html#lus) and the Webster codebase server as part of its standard configuration. This configuration can be changed by providing additional parameters in the GSM startup script.
