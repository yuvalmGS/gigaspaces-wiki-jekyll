...
An SLA-Driven Container, also known as the [Service Grid]({{ page.url | remove: 'net' }}/Service-Grid-Processing-Unit-Container.html), is responsible for abstracting the physical characteristics of the host machines from the application deployment.
...
The Service Grid takes a [pre-defined application-required SLA]({{ page.url | remove: 'net' }}/Service-Grid-Processing-Unit-Container.html#ServiceGridProcessingUnitContainer-SLAPolicy), and makes sure that it is met during deployment and runtime, throughout the application's life-cycle.
...
The [Grid Service Manager (GSM)]({{ page.url | remove: 'net' }}/The-Grid-Service-Manager.html), is a special infrastructure service, responsible for managing the Service Grid containers. The GSM accepts user deployment and undeployment requests, and provisions the Service Grid cloud accordingly.
...
The GSM service usually contains the [Lookup Service]({{ page.url | remove: 'net' }}/Lookup-Service-Configuration.html) and the Webster codebase server as part of its standard configuration. This configuration can be changed by providing additional parameters in the GSM startup script.
