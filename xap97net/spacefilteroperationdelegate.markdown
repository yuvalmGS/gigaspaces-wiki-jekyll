...
An depanlinkISpaceFiltertengahlink./ispacefilter-interface.htmlbelakanglink implementation that acts as an adapter, delegating the execution of the filter life-cycle methods and specific operation, to pluggable reflection-based methods.
...
To use this class as the filter, simply use the `AttributeSpaceFilterConfigFactory` to create a depanlinkSpaceFilterConfigtengahlink./spacefilterconfig-class.htmlbelakanglink instance, and use when starting a space depanlinkintegrated with the space filtertengahlink./implementing-and-using-a-space-filter.html#Integrating the Space Filter with a spacebelakanglink.
...
The `AttributeSpaceFilterConfigFactory` creates a depanlinkSpaceFilterConfigtengahlink./spacefilterconfig-class.htmlbelakanglink instance, using a fully constructed `SpaceFilterOperationDelegate` as its Filter instance. The `SpaceFilterOperationDelegate` acts as the depanlinkISpaceFiltertengahlink./ispacefilter-interface.htmlbelakanglink implementation, and delegates the filter operation to the `SpaceAttributeFilter` instance.
...
- A single parameter: the parameter can either be an depanlinkISpaceFilterEntrytengahlink./ispacefilterentry-interface.htmlbelakanglink, or the actual template object wrapped by the entry.
...
{% lampon %} When your filter method needs to update the entry itself, depanlinkISpaceFilterEntrytengahlink./ispacefilterentry-interface.htmlbelakanglink should be used, and a call to `UpdateObject` needs to be made.
